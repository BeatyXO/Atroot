...
class ATROOTFirewallV2(gl.Contract):
    ...
    def create_proposal(self, target: Address, method: str, release: str, nonce: u256, charter_version: str, evidence_url: str, evidence_digest: str, intent: str) -> u256:
        ...
        if not release or not evidence_digest or not intent:
            raise gl.vm.UserError("EXPECTED: exact action and evidence commitment required")
        action_digest = digest(str(target) + "|" + method + "|" + release + "|" + str(int(nonce)) + "|" + charter_version)
        proposal_id = self.next_proposal_id
        self.next_proposal_id = u256(int(proposal_id) + 1)
        self.proposals[proposal_id] = FirewallProposal(proposal_id, gl.message.sender_address, target, method, release, nonce, charter_version, action_digest, evidence_url, evidence_digest, intent[:MAX_TEXT], PROPOSED, 0, "", u256(0), now(), u256(0))
    ...
    def review_proposal(self, proposal_id: u256) -> dict:
        ...
        def leader() -> dict:
            ...
            body = response.body.decode("utf-8")[:MAX_TEXT]
    ...
    @gl.public.write
    def challenge_proposal(self, proposal_id: u256, reason: str) -> bool:
        p = self._proposal(proposal_id)
        if p.status != CHALLENGE_WINDOW or int(now()) > int(p.challenge_deadline):
            raise gl.vm.UserError("EXPECTED: challenge window is closed")
        if not reason or len(reason) > 240:
            raise gl.vm.UserError("EXPECTED: bounded challenge reason required")
        p.status = CANCELED
        p.rationale = "CHALLENGED: " + reason
        self.proposals[proposal_id] = p
        return True
    ...
    def execute_proposal(self, proposal_id: u256) -> bool:
        ...
        target.emit(on="finalized").apply_release(self._key(proposal_id), p.release, p.nonce)
    ...
    def confirm_execution(self, proposal_id: u256) -> dict:
        ...
        if str(state.get("release", "")) != p.release or int(state.get("release_nonce", 0)) != int(p.nonce):
            p.status = FAILED
            self.proposals[proposal_id] = p
